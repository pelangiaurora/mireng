...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; color: string }> = {
    ...
    need_info: { label: 'Perlu Info',   color: 'bg-blue-50 text-blue-700 border-blue-200' },
  };
...
function ReviewModal({ app, onClose, onDone }: {
  ...
  const [status, setStatus] = useState<'approved' | 'rejected' | 'need_info'>('approved');
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await api.patch(`/admin/seller-applications/${app.id}/review`, {
        status,
        adminNotes,
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl w-full max-w-md p-6 space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h3 className="font-black text-gray-900">Review Permohonan</h3>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 space-y-2 text-xs">
          <p className="font-semibold text-gray-700">Detail Permohonan:</p>
          <p className="text-gray-600"><strong>Tipe:</strong> {app.sellerTypeRequested}</p>
          <p className="text-gray-600"><strong>Alasan:</strong> {app.reason}</p>
          {app.estimatedProducts && (
            <p className="text-gray-600"><strong>Estimasi Produk:</strong> {app.estimatedProducts}</p>
          )}
          {app.estimatedRevenue && (
            <p className="text-gray-600"><strong>Estimasi Omset:</strong> Rp {Number(app.estimatedRevenue).toLocaleString('id-ID')}</p>
          )}
          {app.experienceYears && (
            <p className="text-gray-600"><strong>Pengalaman:</strong> {app.experienceYears} tahun</p>
          )}
          {app.documentKtp && (
            <a href={app.documentKtp} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 text-blue-600 hover:underline mt-2">
              <Eye size={12} /> Lihat KTP
            </a>
          )}
          {app.documentSupport && (
            <a href={app.documentSupport} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 text-blue-600 hover:underline">
              <Eye size={12} /> Dokumen Pendukung
            </a>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(['approved', 'rejected', 'need_info'] as const).map(s => (
              <button key={s} onClick={() => setStatus(s)}
                ...
                {s === 'approved' ? '✓ Approve' : s === 'rejected' ? '✗ Reject' : '? Info'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <textarea value={adminNotes} onChange={e => setAdminNotes(e.target.value)}
            placeholder="Catatan untuk pemohon..." rows={2}
        ...
        {status === 'rejected' && (
          <div>
            ...
            <textarea value={rejectionReason} onChange={e => setRejectionReason(e.target.value)}
              placeholder="Jelaskan mengapa permohonan ditolak..." rows={2}
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-gray-900 transition-all resize-none"
            />
          </div>
        )}
...
export default function AdminSellerApplicationsPage() {
  ...
  const fetchApps = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/seller-applications?status=${filter}`);
    ...
    <div className="min-h-screen bg-gray-50/50">
      {selected && (
        <ReviewModal
          ...
          onDone={() => { setSelected(null); fetchApps(); }}
        />
      )}
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-base font-bold text-gray-900">Permohonan Seller</h1>
            <p className="text-xs text-gray-400">{apps.length} permohonan</p>
          </div>
          <button onClick={fetchApps} className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {['pending', 'approved', 'rejected', 'need_info'].map(s => (
            <button key={s} onClick={() => setFilter(s)}
              ...
              {s === 'pending' ? 'Pending' : s === 'approved' ? 'Approved' : s === 'rejected' ? 'Rejected' : 'Perlu Info'}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            ...
            <p className="text-sm text-gray-400">Tidak ada permohonan {filter}</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-bold text-gray-900 text-sm">
                        {(app as any).user?.email ?? app.userId}
                      </p>
                      <StatusBadge status={app.status} />
                    </div>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="text-xs text-gray-400 capitalize">{app.sellerTypeRequested}</span>
                      <span className="text-gray-200">·</span>
                      <span className="text-xs text-gray-400">
                        {new Date(app.submittedAt).toLocaleDateString('id-ID')}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1.5 line-clamp-2">{app.reason}</p>
                    {app.adminNotes && (
                      <p className="text-xs text-gray-500 mt-1.5 bg-gray-50 rounded-lg px-3 py-1.5">
                        Catatan: {app.adminNotes}
                      </p>
                    )}
                  </div>
                  {app.status === 'pending' || app.status === 'need_info' ? (
