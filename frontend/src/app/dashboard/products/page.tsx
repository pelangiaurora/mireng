...
import {
  Plus, Search, Edit2, Trash2, X, Check, AlertTriangle,
...
function StatCard({ icon, label, value, sub, accent }: { icon: React.ReactNode; label: string; value: string; sub?: string; accent: string }) {
  return (
    <div className="relative bg-white rounded-2xl border border-gray-100 p-5 overflow-hidden hover:shadow-lg transition-all duration-300">
      ...
      {sub && <p className="text-xs text-gray-400 mt-1 flex items-center gap-1"><ArrowUpRight size={11} className="text-emerald-500" />{sub}</p>}
    </div>
  );
}

function ProductRow({ product, onEdit, onDelete }: { product: Product; onEdit: (p: Product) => void; onDelete: (p: Product) => void }) {
  ...
  return (
    <tr className="group border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
      ...
      <td className="py-3.5 px-3"><span className="text-sm font-bold text-gray-900">{fmt(product.price)}</span></td>
      ...
      <td className="py-3.5 px-3">
        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${product.isActive ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-gray-100 text-gray-500 border border-gray-200"}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${product.isActive ? "bg-emerald-500" : "bg-gray-400"}`} />
          {product.isActive ? "Aktif" : "Nonaktif"}
        </span>
      </td>
      <td className="py-3.5 px-3"><span className="text-xs text-gray-400">{product.images?.length ?? 0} foto</span></td>
      <td className="py-3.5 px-3"><span className="text-xs text-gray-400">{fmtDate(product.createdAt)}</span></td>
      <td className="py-3.5 pl-3 pr-6">
        <div className="relative flex justify-end">
          <button onClick={() => setMenuOpen(v => !v)} className="p-1.5 rounded-lg hover:bg-gray-200 transition-colors opacity-0 group-hover:opacity-100">
...
function ProductModal({ product, onClose, onSave }: { product: Product | null; onClose: () => void; onSave: () => void }) {
  ...
  const set = (k: string, v: string | boolean) => { setForm(p => ({ ...p, [k]: v })); setError(""); };

  const handleSave = async () => {
    if (!form.name.trim()) { setError("Nama produk wajib diisi"); return; }
    if (!form.price || isNaN(Number(form.price))) { setError("Harga harus berupa angka"); return; }
    setSaving(true);
    try {
      const body = {
        name: form.name,
      ...
      if (isEdit) { await api.patch(`/products/${product!.id}`, body); }
      else { await api.post("/products", body); }
      onSave();
    } catch (e: any) {
      const msg = e?.response?.data?.message;
      setError(Array.isArray(msg) ? msg.join(", ") : msg || "Gagal menyimpan produk");
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{isEdit ? `ID: ${product!.id.slice(0, 8)}...` : "Isi detail produk di bawah"}</p>
          </div>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto">
          ...
          {/* Nama */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Nama Produk</label>
            <input value={form.name} onChange={e => set("name", e.target.value)}
              placeholder="cth. Sepatu Nike Air Max" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-gray-900 transition-all" />
          </div>

          {/* Tipe Produk */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Tipe Produk</label>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => set("description", e.target.value)}
              placeholder="Jelaskan detail produk..." rows={3}
          ...
          {/* Harga */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Harga (Rp)</label>
            <div className="relative">
              ...
              <input type="number" value={form.price} onChange={e => set("price", e.target.value)} placeholder="79000"
                className="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-gray-900 transition-all" />
            </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex-shrink-0">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100 transition-colors">Batal</button>
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Menyimpan...</>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
        ...
        <p className="text-sm text-gray-500 mt-1.5"><span className="font-semibold text-gray-700">"{product.name}"</span> akan dihapus permanen.</p>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50">Batal</button>
    ...
    <div className="min-h-screen bg-gray-50/50">
      <div className="bg-white border-b border-gray-100 sticky top-16 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Kelola semua produk kamu di sini</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={<Package size={18} />} label="Total Produk" value={String(products.length)} sub={`${activeCount} aktif`} accent="bg-blue-500" />
          <StatCard icon={<Eye size={18} />} label="Produk Aktif" value={String(activeCount)} sub="tampil di marketplace" accent="bg-emerald-500" />
          <StatCard icon={<DollarSign size={18} />} label="Total Nilai" value={fmt(totalRevenue)} sub="nilai semua produk" accent="bg-violet-500" />
          <StatCard icon={<BarChart2 size={18} />} label="Tdk Aktif" value={String(products.length - activeCount)} sub="tersembunyi" accent="bg-amber-500" />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-sm font-semibold text-gray-600">{search ? `Tidak ada produk "${search}"` : "Belum ada produk"}</p>
              <p className="text-xs text-gray-400 mt-1">{search ? "Coba kata kunci lain" : "Klik 'Tambah Produk' untuk mulai"}</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map(p => (
                    <ProductRow key={p.id} product={p} onEdit={setEditProduct} onDelete={setDeleteProduct} />
                  ))}
                </tbody>
              </table>
            </div>
          ...
          {filtered.length > 0 && (
            <div className="px-6 py-3 border-t border-gray-50">
              <p className="text-xs text-gray-400">Menampilkan <span className="font-semibold text-gray-600">{filtered.length}</span> dari <span className="font-semibold text-gray-600">{products.length}</span> produk</p>
            </div>
          )}
        </div>
      </div>
      ...
      {editProduct !== undefined && (
        <ProductModal product={editProduct} onClose={() => setEditProduct(undefined)} onSave={() => { setEditProduct(undefined); fetchProducts(); }} />
      )}
      {deleteProduct && (
